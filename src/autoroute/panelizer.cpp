...
// NOTE: OCRAFontName previously lived in src/utils/misc.h, which the panelizer
// SVG-text generation relied on. That constant was removed from develop's
// misc.h, so it is reintroduced here as a file-local constant to keep this
// module self-contained and avoid touching a widely-included core header.
...
void Panelizer::panelize(FApplication * app, const QString & panelFilename, bool customPartsOnly)
{
	...
	foreach (PlanePair * planePair, planePairs) {
		...
		// NOTE: QPrinter::paperRect() (no-arg) and setPaperSize() were removed in Qt 6.
		// paperRect(Unit) and setPageSize(QPageSize) exist in Qt 5.3+ and Qt 6, so use
		// them unconditionally. DevicePixel keeps the same device-pixel semantics the
		// old no-arg paperRect() returned.
		QRectF paperRect = printer.paperRect(QPrinter::DevicePixel);
		QSizeF psize((target.width() + paperRect.width() - printer.width()) / res,
		             (target.height() + paperRect.height() - printer.height()) / res);
		printer.setPageSize(QPageSize(psize, QPageSize::Inch));
...
void Panelizer::addOptional(int optionalCount, QList<PanelItem *> & refPanelItems, QList<PanelItem *> & insertPanelItems, PanelParams & panelParams, QList<PlanePair *> & planePairs)
{
	...
	while (optionalCount > 0) {
		...
		// NOTE: qrand()/RAND_MAX was removed in Qt 6. QRandomGenerator (Qt 5.10+) yields
		// a uniform double in [0, 1); scaling by pool reproduces the original index range.
		int ix = qFloor(QRandomGenerator::global()->generateDouble() * pool);
		int soFar = 0;
		foreach (PanelItem * panelItem, copies) {
...
MainWindow * Panelizer::inscribeBoard(QDomElement & board, QHash<QString, QString> & fzzFilePaths, FApplication * app, QDir & fzDir, bool drc, bool noMessages, QDir & copyDir)
{
	...
	foreach (ItemBase * boardItem, boards) {
		...
		if (boardItem->prop("layers").compare("1") == 0) {
			mainWindow->swapLayers(boardItem, 2, "");
...
int Panelizer::checkDonuts(MainWindow * mainWindow, bool displayMessage) {
	...
	if (displayMessage && donuts.count() > 0) {
		...
		mainWindow->pcbView()->selectItems(QList<ItemBase *>(itemBases.begin(), itemBases.end()));
